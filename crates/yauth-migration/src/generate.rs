//! Migration file generators for diesel, sqlx, and raw SQL.
...
pub fn generate_add_plugin(
    ...
    let files = match config.migration.orm {
        ...
        crate::Orm::Raw => generate_raw_files(migrations_dir, &migration_name, up_sql, down_sql)?,
    };
...
/// Generate raw SQL files.
fn generate_raw_files(
    migrations_dir: &Path,
    name: &str,
    up_sql: &str,
    down_sql: &str,
) -> Result<Vec<(PathBuf, String)>, GenerateError> {
    let up_path = migrations_dir.join(format!("{name}_up.sql"));
    let down_path = migrations_dir.join(format!("{name}_down.sql"));
...
mod tests {
    ...
    fn generate_init_raw_mode() {
        ...
        let result = generate_init(&config).unwrap();
        assert_eq!(result.files.len(), 2);
        assert!(result.files[0].0.to_string_lossy().contains("_up.sql"));
        assert!(result.files[1].0.to_string_lossy().contains("_down.sql"));
    }
