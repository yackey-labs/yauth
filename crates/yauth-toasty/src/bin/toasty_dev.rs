...
fn load_history() -> History {
    match fs::read_to_string(history_path()) {
        Ok(content) => toml::from_str(&content).unwrap_or(History {
            migrations: Vec::new(),
        }),
        Err(_) => History {
            migrations: Vec::new(),
...
async fn cmd_generate(name: &str) -> Result<()> {
    ...
    let previous = if history.migrations.is_empty() {
        toasty::schema::db::Schema::default()
    } else {
        // For subsequent migrations, we'd load the previous snapshot.
        // For now, we use empty schema since this is the initial generation.
        // Future: deserialize from snapshots/NNNN_snapshot.toml
        toasty::schema::db::Schema::default()
    };
    ...
    updated_history.migrations.push(MigrationEntry {
        name: migration_name.clone(),
        checksum,
        created_at: now.to_string(),
    });
    let history_content = toml::to_string_pretty(&updated_history)?;
