//! Cross-backend repository conformance test suite for Toasty backends.
//!
//! Runs identical assertions against every available Toasty backend:
//! - toasty_pg: if DATABASE_URL env var is set (feature: postgresql)
//! - toasty_mysql: if MYSQL_DATABASE_URL env var is set (feature: mysql)
//! - toasty_sqlite: always available via sqlite::memory: (feature: sqlite)
//!
//! ```text
//! cargo test --features postgresql,mysql,sqlite,full --test conformance -- --test-threads=1
//! ```
...
async fn shared_pg_repos() -> Option<Repositories> {
    PG_REPOS
        .get_or_init(|| async {
            ...
            match ToastyPgBackend::new(&url).await {
                Ok(backend) => {
                    // push_schema may fail if tables already exist (e.g., shared DB
                    // with diesel/sqlx). We ignore the error and proceed.
                    let _ = backend.create_tables().await;
                    Some(backend.repositories())
                }
                Err(e) => {
                    eprintln!("toasty_pg: connection failed: {e}");
                    None
                }
            }
        })
...
async fn shared_mysql_repos() -> Option<Repositories> {
    MYSQL_REPOS
        .get_or_init(|| async {
            ...
            match ToastyMysqlBackend::new(&url).await {
                Ok(backend) => {
                    let _ = backend.create_tables().await;
                    Some(backend.repositories())
                }
                Err(e) => {
                    eprintln!("toasty_mysql: connection failed: {e}");
                    None
                }
            }
        })
...
async fn shared_sqlite_repos() -> Option<Repositories> {
    SQLITE_REPOS
        .get_or_init(|| async {
            ...
            let result: Option<Repositories> =
                match ToastySqliteBackend::new("sqlite::memory:").await {
                    Ok(backend) => {
                        let _ = backend.create_tables().await;
                        Some(backend.repositories())
                    }
                    Err(e) => {
                        eprintln!("toasty_sqlite: connection failed: {e}");
                        None
                    }
                };
            result
...
async fn test_backends() -> Vec<(&'static str, Repositories)> {
    ...
    if let Some(repos) = shared_pg_repos().await {
        backends.push(("toasty_pg", repos));
    }

    #[cfg(feature = "mysql")]
    if let Some(repos) = shared_mysql_repos().await {
        backends.push(("toasty_mysql", repos));
    }

    #[cfg(feature = "sqlite")]
    if let Some(repos) = shared_sqlite_repos().await {
        backends.push(("toasty_sqlite", repos));
    }

    if backends.is_empty() {
        panic!(
            "No Toasty backends available. Enable at least one feature (postgresql, mysql, sqlite) and set the appropriate env var."
        );
    }
...
fn user_delete_cascades() {
    shared_runtime().block_on(async {
        for (name, repos) in test_backends().await {
            ...
            // NOTE: Toasty does not support FK CASCADE constraints, so related
            // entities (sessions, passwords, passkeys, api keys) are NOT automatically
            // deleted when the user is deleted. This is a known limitation of the
            // Toasty ORM. Production code should explicitly clean up related entities
            // before or after deleting a user.
        }
    });
}
...
fn large_text_round_trip() {
    shared_runtime().block_on(async {
        for (name, repos) in test_backends().await {
            ...
            let id = Uuid::now_v7();
            let n = now();
            // Toasty uses VARCHAR(191) on MySQL (utf8mb4 InnoDB index limit)
            ...
            assert_eq!(
                ...
                "{name}: 190-char display_name should round-trip exactly (got {} chars)",
