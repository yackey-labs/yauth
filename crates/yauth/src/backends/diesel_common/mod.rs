...
// ── Native SQLite pool impl ──

#[cfg(feature = "diesel-sqlite-backend")]
impl PoolAccess
    for diesel_async_crate::pooled_connection::deadpool::Pool<
        diesel_async_crate::sync_connection_wrapper::SyncConnectionWrapper<
...
/// string match to `RepoError::Conflict`. Used by the libSQL and native SQLite
/// backends where diesel sometimes returns a generic error with the constraint
/// message embedded in the string representation.
#[cfg(any(feature = "diesel-libsql-backend", feature = "diesel-sqlite-backend"))]
