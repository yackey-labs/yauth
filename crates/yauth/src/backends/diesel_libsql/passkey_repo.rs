...
pub(crate) struct LibsqlWebauthnCredential {
    pub id: String,
    pub user_id: String,
    pub name: String,
...
pub(crate) struct LibsqlPasskeyRepo {
    pool: LibsqlPool,
}
impl LibsqlPasskeyRepo {
    pub(crate) fn new(pool: LibsqlPool) -> Self {
        Self { pool }
    }
}
impl sealed::Sealed for LibsqlPasskeyRepo {}
impl PasskeyRepository for LibsqlPasskeyRepo {
    fn find_by_user_id(&self, user_id: Uuid) -> RepoFuture<'_, Vec<domain::WebauthnCredential>> {
        Box::pin(async move {
            ...
            let r: Vec<LibsqlWebauthnCredential> = yauth_webauthn_credentials::table
                .filter(yauth_webauthn_credentials::user_id.eq(&uid))
                .select(LibsqlWebauthnCredential::as_select())
                .load(&mut *c)
                .await
                .map_err(diesel_err)?;
