...
pub(crate) struct MysqlRefreshTokenRepo {
    pool: MysqlPool,
}
...
impl sealed::Sealed for MysqlRefreshTokenRepo {}
impl RefreshTokenRepository for MysqlRefreshTokenRepo {
    fn find_by_token_hash(&self, th: &str) -> RepoFuture<'_, Option<domain::RefreshToken>> {
        ...
        Box::pin(async move {
            ...
            let r = yauth_refresh_tokens::table
                ...
                .select(MysqlRefreshToken::as_select())
                .first(&mut *c)
                .await
                .optional()
                .map_err(diesel_err)?;
    ...
    fn create(&self, input: domain::NewRefreshToken) -> RepoFuture<'_, ()> {
        Box::pin(async move {
            ...
            diesel::sql_query(
                "INSERT INTO yauth_refresh_tokens \
                 (id, user_id, token_hash, family_id, expires_at, revoked, created_at) \
                 VALUES (?, ?, ?, ?, ?, ?, ?)",
            )
            ...
            .bind::<diesel::sql_types::Bool, _>(rev)
