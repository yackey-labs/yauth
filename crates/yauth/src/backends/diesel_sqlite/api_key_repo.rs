...
pub(crate) struct SqliteApiKeyRepo {
    pool: SqlitePool,
}
...
impl sealed::Sealed for SqliteApiKeyRepo {}
impl ApiKeyRepository for SqliteApiKeyRepo {
    ...
    fn list_by_user_id(&self, user_id: Uuid) -> RepoFuture<'_, Vec<domain::ApiKey>> {
        Box::pin(async move {
            ...
            let r: Vec<SqliteApiKey> = yauth_api_keys::table
