...
pub(crate) struct SqliteChallengeRepo {
    pool: SqlitePool,
    ...
    async fn ensure_init(&self) -> Result<(), RepoError> {
        ...
        lazy_init_table(&self.initialized, || async {
            ...
            (*conn)
                ...
                .await
                .map_err(|e| {
                    RepoError::Internal(format!("challenge table init failed: {e}").into())
                })?;
            Ok(())
        })
        .await
...
impl sealed::Sealed for SqliteChallengeRepo {}

impl ChallengeRepository for SqliteChallengeRepo {
    fn set_challenge(
        &self,
        key: &str,
