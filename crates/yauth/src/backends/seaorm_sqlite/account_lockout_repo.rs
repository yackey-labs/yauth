    ...
    fn find_by_user_id(&self, user_id: Uuid) -> RepoFuture<'_, Option<domain::AccountLock>> {
        Box::pin(async move {
            let row = account_locks::Entity::find()
                .filter(account_locks::Column::UserId.eq(user_id.to_string()))
                .one(&self.db)
                .await
                .map_err(sea_err)?;
    ...
    fn increment_failed_count(&self, id: Uuid) -> RepoFuture<'_, ()> {
        Box::pin(async move {
            account_locks::Entity::update_many()
                ...
                .filter(account_locks::Column::Id.eq(id.to_string()))
                .exec(&self.db)
                .await
                .map_err(sea_err)?;
            Ok(())
...
impl UnlockTokenRepository for SeaOrmUnlockTokenRepo {
    ...
    fn delete(&self, id: Uuid) -> RepoFuture<'_, ()> {
        Box::pin(async move {
            unlock_tokens::Entity::delete_many()
                .filter(unlock_tokens::Column::Id.eq(id.to_string()))
                .exec(&self.db)
                .await
                .map_err(sea_err)?;
            Ok(())
    ...
    fn delete_all_for_user(&self, user_id: Uuid) -> RepoFuture<'_, ()> {
        Box::pin(async move {
            unlock_tokens::Entity::delete_many()
                .filter(unlock_tokens::Column::UserId.eq(user_id.to_string()))
                .exec(&self.db)
                .await
                .map_err(sea_err)?;
            Ok(())
