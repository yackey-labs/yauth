    ...
    fn create(&self, input: domain::NewOauth2Client) -> RepoFuture<'_, ()> {
        Box::pin(async move {
            let model = oauth2_clients::ActiveModel {
                ...
                redirect_uris: Set(serde_json::to_string(&input.redirect_uris).unwrap_or_default()),
                client_name: Set(input.client_name),
                grant_types: Set(serde_json::to_string(&input.grant_types).unwrap_or_default()),
                scopes: Set(input
                    .scopes
    ...
    fn update_scopes(&self, id: Uuid, scopes: Option<serde_json::Value>) -> RepoFuture<'_, ()> {
        Box::pin(async move {
            consents::Entity::update_many()
                .col_expr(
                    consents::Column::Scopes,
                    Expr::value(scopes.map(|v| serde_json::to_string(&v).unwrap_or_default())),
                )
