...
impl PasskeyRepository for SeaOrmPasskeyRepo {
    ...
    fn find_by_id_and_user(
        ...
        Box::pin(async move {
            let row = passkeys::Entity::find_by_id(id.to_string())
                .filter(passkeys::Column::UserId.eq(user_id.to_string()))
                .one(&self.db)
                .await
                .map_err(sea_err)?;
    ...
    fn create(&self, input: domain::NewWebauthnCredential) -> RepoFuture<'_, ()> {
        Box::pin(async move {
            let model = passkeys::ActiveModel {
                ...
                credential: Set(serde_json::to_string(&input.credential).unwrap_or_default()),
    ...
    fn delete(&self, id: Uuid) -> RepoFuture<'_, ()> {
        Box::pin(async move {
            passkeys::Entity::delete_many()
                .filter(passkeys::Column::Id.eq(id.to_string()))
                .exec(&self.db)
                .await
                .map_err(sea_err)?;
            Ok(())
