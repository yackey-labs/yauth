...
impl EmailVerificationRepository for SeaOrmEmailVerificationRepo {
    ...
    fn delete(&self, id: Uuid) -> RepoFuture<'_, ()> {
        Box::pin(async move {
            email_verifications::Entity::delete_many()
                .filter(email_verifications::Column::Id.eq(id.to_string()))
                .exec(&self.db)
                .await
                .map_err(sea_err)?;
            Ok(())
    ...
    fn delete_all_for_user(&self, user_id: Uuid) -> RepoFuture<'_, ()> {
        Box::pin(async move {
            email_verifications::Entity::delete_many()
                .filter(email_verifications::Column::UserId.eq(user_id.to_string()))
                .exec(&self.db)
                .await
                .map_err(sea_err)?;
            Ok(())
    ...
    fn delete_unused_for_user(&self, user_id: Uuid) -> RepoFuture<'_, ()> {
        Box::pin(async move {
            password_resets::Entity::delete_many()
                .filter(password_resets::Column::UserId.eq(user_id.to_string()))
                .filter(password_resets::Column::UsedAt.is_null())
                .exec(&self.db)
                .await
                .map_err(sea_err)?;
