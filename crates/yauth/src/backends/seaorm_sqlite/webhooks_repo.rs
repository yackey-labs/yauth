...
impl WebhookRepository for SeaOrmWebhookRepo {
    fn find_by_id(&self, id: Uuid) -> RepoFuture<'_, Option<domain::Webhook>> {
        Box::pin(async move {
            let row = webhooks::Entity::find_by_id(id.to_string())
                .one(&self.db)
                .await
                .map_err(sea_err)?;
    ...
    fn create(&self, input: domain::NewWebhook) -> RepoFuture<'_, ()> {
        Box::pin(async move {
            let model = webhooks::ActiveModel {
                ...
                events: Set(serde_json::to_string(&input.events).unwrap_or_default()),
    ...
    fn update(&self, id: Uuid, changes: domain::UpdateWebhook) -> RepoFuture<'_, domain::Webhook> {
        Box::pin(async move {
            let existing = webhooks::Entity::find_by_id(id.to_string())
                .one(&self.db)
                .await
                .map_err(sea_err)?
            ...
            if let Some(events) = changes.events {
                model.events = Set(serde_json::to_string(&events).unwrap_or_default());
            }
    ...
    fn delete(&self, id: Uuid) -> RepoFuture<'_, ()> {
        Box::pin(async move {
            webhooks::Entity::delete_many()
                .filter(webhooks::Column::Id.eq(id.to_string()))
                .exec(&self.db)
                .await
                .map_err(sea_err)?;
            Ok(())
    ...
    fn create(&self, input: domain::NewWebhookDelivery) -> RepoFuture<'_, ()> {
        Box::pin(async move {
            let model = webhook_deliveries::ActiveModel {
                id: Set(input.id.to_string()),
                webhook_id: Set(input.webhook_id.to_string()),
                event_type: Set(input.event_type),
                payload: Set(serde_json::to_string(&input.payload).unwrap_or_default()),
