...
//!
//! Uses compile-time `sqlx::query!()` / `sqlx::query_as!()` macros for
//! static SQL, with runtime `sqlx::query()` for dynamic queries (conditional
//! SET clauses in updates). Postgres-specific: native UUID, RETURNING,
//! ILIKE, JSONB, TIMESTAMPTZ, `$1` placeholders, `make_interval()`,
//! UNLOGGED tables.
//!
//! Requires `DATABASE_URL` env var at compile time pointing to a PG database
//! with the yauth schema, OR `SQLX_OFFLINE=true` with a cached `.sqlx/`
//! directory generated by `cargo sqlx prepare`.
