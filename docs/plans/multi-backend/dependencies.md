...
- **Fallible construction**: `DieselPgBackend::new()` returns `Result<Self, RepoError>`, not `Self`. Pool creation, schema validation, and instrumentation setup can all fail — make that explicit in the type system rather than panicking.
