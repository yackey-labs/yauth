...
3. **ID generation**: The in-memory backend must generate UUIDs for new entities the same way — `Uuid::now_v7()`. Some entities have `id` as a caller-provided field in the `NewXxx` struct, others don't. Match the existing patterns from M1's domain types.
