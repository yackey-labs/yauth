...
11. **`InMemoryBackend::new()` is infallible**: Unlike `DieselBackend::new()` which returns `Result` (pool creation can fail), `InMemoryBackend::new()` just initializes empty HashMaps — nothing to fail. Return `Self` directly. This is a nice ergonomic signal that the in-memory backend is zero-config.
