...
- **`PUT`/`PATCH` will only repoint a `userName` into a domain your org has VERIFIED.** `userName` is the user's *global* login identity — the address `/auth/forgot-password` resolves — and a SCIM key is scoped to one org, not to the account. Previously the only check was "is this address taken by someone else", so an address nobody held sailed through: an org admin could point any member's login at a mailbox they controlled and redeem a password reset, inheriting that member's global role and every other org they belong to. A rename to an address outside a verified `OrganizationDomain` of your org now returns 409 `uniqueness`. This is the same bar `POST /Users` already applies to adoption. **Cross-domain renames via SCIM require you to add and verify the domain first.**
- **A genuine rename terminates every session and revokes every refresh token** for that user. The account is now reachable from a different mailbox, so nothing minted under the old one may keep working. A routine profile sync that re-sends the same `userName` is *not* a rename and leaves credentials alone.
- **`userName` is normalised to lower case** before it is stored or looked up. `yauth_users.email` is unique over plain text and every other credential path lower-cases, so SCIM's mixed case used to slip past `GetUserByEmail` entirely — the cross-tenant adoption guard, the collision check and POST idempotency all silently did not run, and a second global account was minted for an address another tenant already owned. `userName` values that are empty or not an address are rejected with 400 `invalidValue` rather than written.
- **`POST /Users` never overwrites the role on a membership that already exists.** SCIM provisions people; roles are administered through the organizations API. Re-POSTing an existing member still updates their status (so re-provisioning works) but leaves `owner`/`admin` intact. New memberships still default to `member`.
