import type { YAuthClient } from "@yackey-labs/yauth-go-client";
import type { AuthUser } from "@yackey-labs/yauth-go-shared";
import {
	type App,
	type InjectionKey,
	inject,
...
export interface YAuthPluginOptions {
	...
	client?: YAuthClient;
	/** Base URL for the auth API. When provided without `client`, a default client is lazily created via `@yackey-labs/yauth-go-client`. */
	baseUrl?: string;
}
...
export const YAuthPlugin = {
	install(app: App, options: YAuthPluginOptions) {
		...
		if (options.client) {
			...
			import("@yackey-labs/yauth-go-client").then((mod) => {
